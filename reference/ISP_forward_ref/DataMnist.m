...
Ep_s=1.2; Ep_l=1.7;  % permittivity range

N_t=12000; % total data number;
E_s=zeros(N_rec,N_inc,N_t);
Pro_Para=cell(1,N_t);

Contrast = zeros(N_t, Mx,Mx);
for nn=1:N_t
...
epsil = contrast(nn,:,:);
epsil= reshape(epsil,[64,64]); 
bool = epsil>1.2;
epsil(bool)=1.2+0.5*rand;
[epsil2,Para] = annulus_gen_rand(MAX,Mx,1,[0.2,0.4],[1.2,1.7]);
bool2 = epsil2>1.1;
epsil(bool2)=1.2+0.5*rand;
...
[theta,rho] = meshgrid(theta_tmp,rrr); theta = theta(:); rho = rho(:);
...
if bool_plane == 1
    ...
    rin=3;
    theta_inc = linspace(0, 2*pi, N_inc+1); theta_inc(end) = []; theta_inc = theta_inc(:);
    [theta_t,rho_t] = meshgrid(theta_inc,rin); theta_t = theta_t(:); rho_t = rho_t(:);
    ...
    [xt1, xt2] = meshgrid(x, x_t);
    [yt1, yt2] = meshgrid(y, y_t);

    rh = sqrt((xt1-xt2).^2 +(yt1-yt2).^2);
    TT = i*k_0*eta_0 *(i/4)*besselh(0,1,k_0*rh);
...
clearvars -except E_s Pro_Para R_mat Contrast E_i;


save('Phaseless_MnistRotaCir_12000PS2.mat')
